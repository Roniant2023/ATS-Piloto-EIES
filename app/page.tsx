...
export default function Page() {
  ...
  try {
    ...
    const hasUsefulContent =
      !!String(proc?.brief?.scope || "").trim() ||
      (Array.isArray(proc?.brief?.mandatory_permits) && proc.brief.mandatory_permits.length > 0) ||
      (Array.isArray(proc?.brief?.mandatory_steps) && proc.brief.mandatory_steps.length > 0) ||
      (Array.isArray(proc?.brief?.stop_work) && proc.brief.stop_work.length > 0) ||
      (Array.isArray(proc?.brief?.restrictions) && proc.brief.restrictions.length > 0) ||
      (Array.isArray(proc?.brief?.critical_controls?.engineering) &&
        proc.brief.critical_controls.engineering.length > 0) ||
      (Array.isArray(proc?.brief?.critical_controls?.administrative) &&
        proc.brief.critical_controls.administrative.length > 0) ||
      (Array.isArray(proc?.brief?.critical_controls?.ppe) &&
        proc.brief.critical_controls.ppe.length > 0) ||
      (Array.isArray(proc?.excerpts) && proc.excerpts.length > 0);

    if (!hasUsefulContent) {
      return {
        ok: false,
        fileName: file.name,
        error: "Procedimiento sin contenido utilizable",
        details: "El backend respondió, pero no extrajo contenido útil del documento.",
      };
    }
  ...
  try {
    ...
    for (const file of selectedFiles) {
      ...
      if (r.ok && r.procedure) {
        procs.push(r.procedure);
      }

      // actualiza en vivo sin perder los buenos
      setProcedureResults([...results]);
      setProcedureRefs([...procs]);
    }

    const okCount = results.filter((r) => r.ok && r.procedure).length;
    const failCount = results.filter((r) => !r.ok || !r.procedure).length;

    if (okCount === 0) {
      setUiError(
        "No se pudo estructurar ningún procedimiento. Revisa los archivos o intenta procesarlos de uno en uno."
      );
      setUiInfo(null);
      return;
    }
    ...
    if (failCount > 0) {
      setUiInfo(
        `Se procesaron ${okCount} procedimiento(s) correctamente y ${failCount} quedaron con observaciones. Puedes generar el ATS con los válidos.`
      );
    } else {
      setUiInfo(`Procedimientos listos: ${okCount}.`);
    }
  } catch (err: any) {
    setUiError(`Error procesando procedimientos: ${String(err?.message || err)}`);
  } finally {
    setUploading(false);
  }
}
