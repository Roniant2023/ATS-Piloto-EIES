...
function computeStopWorkTriggers(payload: any) {
  ...
  if (temperatureC !== null && temperatureC >= 35) {
    triggers.push("Temperatura elevada (≥35°C): detener si no hay pausas, hidratación, sombra y monitoreo (estrés térmico).");
  }
...
/* =========================
   ...
   - soporta varias formas del body
========================= */
function extractLessonLearnedRef(body: any) {
  // formas comunes:
  // 1) body.lesson_learned_brief
  // 2) body.lesson_learned_ref
  // 3) body.lesson?.lesson_learned_brief
  // 4) body.data?.lesson_learned_brief (si guardaste respuesta completa)
  // 5) body.lessonLearned?.lesson_learned_brief (camelcase)
  const ll =
...
function deriveChecklistDeterministic(estrella: any, body: any): ChecklistActionsPayload {
  ...
  // ✅ robusto a variantes "Sí/SI/true"
  ...
  // 1) Checks del supervisor: si algún NO -> STOP
  ...
  // 6) Equipos de seguridad -> controles
...
export async function POST(req: Request) {
  try {
    ...
    // - con normalización robusta
    // - y modo flexible opcional
    // =========================
...
`.trim();
    ...
    // 4) Controles derivados del checklist
    ...
    // 4.1) Controles derivados de procedimientos/lección aprendida
    ...
    // 5) Ajustar STOP WORK según checklist
    ...
    // 6) critical_fails -> auto_triggers
